...
chaincodeInvoke () {
	...
	# lets supply it directly as we know it using the "-o" option
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		for nrequest in {1..1000} ; do
			peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -c '{"Args":["invoke","a$nrequest","b$nrequest","1"]}' &
		done
	else
		for nrequest in {1..1000} ; do
			peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -c '{"Args":["invoke","a$nrequest","b$nrequest","1"]}' &
		done
	fi
	res=$?
	cat log.txt
